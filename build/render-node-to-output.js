...
const renderNodeToOutput = (node, output, options) => {
    ...
    if (yogaNode) {
        ...
        if (node.nodeName === 'ink-text') {
            let text = (0, squash_text_nodes_1.default)(node);
            if (text.length > 0) {
                const currentWidth = (0, widest_line_1.default)(text);
                const maxWidth = (0, get_max_width_1.default)(yogaNode);
                if (currentWidth > maxWidth) {
                    const textWrap = (_a = node.style.textWrap) !== null && _a !== void 0 ? _a : 'wrap';
                    text = (0, wrap_text_1.default)(text, maxWidth, textWrap);
                }
        ...
        if (node.nodeName === 'ink-box') {
            (0, render_border_1.default)(x, y, node, output);
            const clipHorizontally = node.style.overflowX === 'hidden';
            const clipVertically = node.style.overflowY === 'hidden';
            if (clipHorizontally || clipVertically) {
                const x1 = clipHorizontally
                    ? x + yogaNode.getComputedBorder(yoga_layout_prebuilt_1.default.EDGE_LEFT)
                    : undefined;
                ...
                const y1 = clipVertically
                    ? y + yogaNode.getComputedBorder(yoga_layout_prebuilt_1.default.EDGE_TOP)
                    : undefined;
                const y2 = clipVertically
                    ? y +
                        yogaNode.getComputedHeight() -
                        yogaNode.getComputedBorder(yoga_layout_prebuilt_1.default.EDGE_BOTTOM)
                    : undefined;
